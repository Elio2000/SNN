...
class SRNN(nn.Module):
    ...
    # 创建对象时自动调用，self代指的是创建的对象
    def __init__(self, n_in, n_rec, n_out, n_t, thr, tau_m, tau_o, b_o, gamma, dt, model, classif, w_init_gain, lr_layer, t_crop, visualize, visualize_light, device, LIF_tpye, Output_type, thro, v_reset, vo_reset):    
        ...
        self.device   = device
        self.LIF_tpye = LIF_tpye
        self.Output_type = Output_type
        
        #Parameters nn.Parameter 是 PyTorch 提供的一个类，用于将张量（Tensor）转换为可学习的参数。通过将张量包装在 nn.Parameter 中，PyTorch 将自动跟踪这个参数，并在反向传播时进行梯度计算和优化。
        self.w_in  = nn.Parameter(torch.Tensor(n_rec, n_in ))
        self.w_in_quan  = nn.Parameter(torch.Tensor(n_rec, n_in ))
        self.w_rec = nn.Parameter(torch.Tensor(n_rec, n_rec))
        self.w_rec_quan = nn.Parameter(torch.Tensor(n_rec, n_rec))
        self.w_out = nn.Parameter(torch.Tensor(n_out, n_rec))
        self.w_out_quan = nn.Parameter(torch.Tensor(n_out, n_rec))
        # 创建了一个大小为 (self.n_rec,) 的零张量；.to(self.device) 方法将张量移动到指定的设备上，可以利用设备的硬件加速。
        self.reg_term = torch.zeros(self.n_rec).to(self.device)
        self.B_out = torch.Tensor(n_out, n_rec).to(self.device)
        # 调用SRNN类的reset_parameters函数
        self.reset_parameters(w_init_gain)
        
        #Visualization
        if self.visu:
            # 开启交互式模式，即允许在图形界面中实时更新图形。
            plt.ion()
            # 建了一个大小为 (2+self.n_out+5,) 的子图列表 self.ax_list，每个子图都共享相同的 x 轴
            # self.fig, self.ax_list = plt.subplots(2+self.n_out+5, sharex=True)
            self.fig, self.ax_list = plt.subplots(2, sharex=True)
            self.fig_out, self.ax_out = plt.subplots(self.n_out, sharex=True)

    def reset_parameters(self, gain):
        # torch.nn.init.kaiming_normal_() 是 PyTorch 提供的一个初始化函数，它使用了 Kaiming He 等人提出的一种初始化方法，用于初始化神经网络中的权重矩阵。该方法可以有效地解决深层神经网络中梯度消失和梯度爆炸的问题。
        torch.nn.init.kaiming_normal_(self.w_in)
        # 通过乘以 gain[0] 来缩放权重矩阵的值
    ...
    def init_net(self, n_b, n_t, n_in, n_rec, n_out):
        ...
        #Hidden state
        self.v  = torch.zeros(n_t,n_b,n_rec).to(self.device) #一个隐藏状态张量 (hidden state)，形状为 (n_t, n_b, n_rec)，其中 n_t 是时间步数，n_b 是 batch 大小，n_rec 是隐藏层的维度。它用于存储模型的隐藏状态信息
    ...
    def t3(self, NoB):
        # NoB = -4
        # print(f"Running with SynapseNoB = {NoB}")
        interval = 2**(0-NoB)
        b = self.w_in.cpu().detach().numpy()
        b = np.round(b / interval) * interval
        self.w_in_quan.data = nn.Parameter(torch.from_numpy(b)).to(self.device).data

        
        b = self.w_rec.cpu().detach().numpy()
        b = np.round(b / interval) * interval
        self.w_rec_quan.data = nn.Parameter(torch.from_numpy(b)).to(self.device).data

        
        b = self.w_out.cpu().detach().numpy()
        b = np.round(b / interval) * interval
        self.w_out_quan.data = nn.Parameter(torch.from_numpy(b)).to(self.device).data

    def neuron_quan(self,t,NoB):
        # print(f"Running with NeuronNoB = {NoB}")
        NoB = 0-NoB
        interval = 0.6*2**NoB
        self.v[t+1] = torch.round(self.v[t+1] / interval) * interval
        self.z[t+1] = torch.round(self.z[t+1] / interval) * interval
        self.vo[t+1] = torch.round(self.vo[t+1] / interval) * interval
    
    def forward(self, x, yt, do_training, args): # x是data，yt是target，do_training是是否训练
        
        self.n_b = x.shape[1]    # Extracting batch size
        self.init_net(self.n_b, self.n_t, self.n_in, self.n_rec, self.n_out)    # Network reset, n_b batch size, n_t time steps in one batch, n_in neuron number of input layer, n_rec, n_out
         
        self.w_rec *= (1 - torch.eye(self.n_rec, self.n_rec, device=self.device))         # Making sure recurrent self excitation/inhibition is cancelled 这个操作的目的是将 self.w_rec 对角线上的元素置零，即消除了循环连接的影响，使得该神经网络模型不再存在自连接或自循环的权重。
        
        self.t3(args.SynapseNoB) #对权重进行量化

        if self.LIF_tpye == True:        
            for t in range(self.n_t-1):     # Computing the network state and outputs for the whole sample duration
                ...
                self.v[t+1]  = (self.alpha * self.v[t] + torch.mm(self.z[t], self.w_rec_quan.t()) + torch.mm(x[t], self.w_in_quan.t())) - self.z[t]*self.thr
                self.neuron_quan(t,args.NeuronNoB)
                #   self.v[t+1] 的更新过程可以看作是包含了衰减、反馈连接、输入连接和spike后减掉阈值的综合效果
                self.z[t+1]  = (self.v[t+1] > self.thr).float()
                self.neuron_quan(t,args.NeuronNoB)

                if self.Output_type == True:
                    self.vo[t+1] = self.kappa * self.vo[t] + torch.mm(self.z[t+1], self.w_out_quan.t()) + self.b_o # b_o是Bias of the output layer
                else:
                    self.vo[t+1] = self.kappa * self.vo[t] + torch.mm(self.z[t+1], self.w_out_quan.t()) + self.b_o - self.zo[t]*self.thro
                self.neuron_quan(t,args.NeuronNoB)

                if self.Output_type == False:
                    self.zo[t+1] = (self.vo[t+1] > self.thro).float()
                    self.neuron_quan(t,args.NeuronNoB)
        else:
            for t in range(self.n_t-1):     # Computing the network state and outputs for the whole sample duration
                ...
                # Forward pass - Hidden state:  v: recurrent layer membrane potential
                #                Visible state: z: recurrent layer spike output, vo: output layer membrane potential (yo incl. activation function)
                update = self.alpha * self.v[t] + torch.mm(self.z[t], self.w_rec_quan.t()) + torch.mm(x[t], self.w_in_quan.t())
                self.v[t+1] = torch.where(self.z[t] == 1, self.v_reset , update)
                self.neuron_quan(t,args.NeuronNoB)
                #   self.v[t+1] 的更新过程可以看作是包含了衰减、反馈连接、输入连接和spike后减掉阈值的综合效果
                self.z[t+1]  = (self.v[t+1] > self.thr).float()
                self.neuron_quan(t,args.NeuronNoB)
                ...
                if self.Output_type == True:
                    ...
                    updateo = self.kappa * self.vo[t] + torch.mm(self.z[t+1], self.w_out_quan.t()) + self.b_o
                    self.vo[t+1] = torch.where(self.zo[t+1] == 1, self.vo_reset, updateo)
                self.neuron_quan(t,args.NeuronNoB)
        ...
        if self.classif:        #Apply a softmax function for classification problems
            yo = F.softmax(self.vo,dim=2)   #在第二个维度上（output channel）上进行softmax操作
        else:
            yo = self.vo

        if do_training:
            self.grads_batch(x, yo, yt) #x是data，yo是output layer的membrane voltage，yt是target
    ...
    def grads_batch(self, x, yo, yt):
        ...
        # trace_in = sum(α^(t-t')*xt')
        trace_in    = torch.einsum('tbr,brit->brit', h, trace_in )     #   将h的第一个维度（时间）与trace_in的第一个维度（批次）进行元素级乘积，并根据指定的维度顺序重新排列结果。最终的输出形状将与trace_in相同，即(batch_size, self.n_rec, self.n_in, self.n_t) #n_b, n_rec, n_in , n_t 
    ...
    def update_plot(self, x, z, yo, yt, L, trace_reg, trace_in, trace_rec, trace_out):
        ...
        # Clear the axis to print new plots
        for k in range(self.ax_out.shape[0]):
            ax = self.ax_out[k]
            ax.clear()
        ...
        for i in range(self.ax_list.shape[0]):
            ax = self.ax_list[i]
            ax.set_xlim([0, self.n_t])
            ax.set_xlabel('Time in ms')
